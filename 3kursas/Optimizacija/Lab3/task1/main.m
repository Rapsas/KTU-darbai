...
for i=1:max_iterations
    ...
    % calculate the gradient of function g1
    grad_g = [2 -2*x0(2)];
    
    % if the second constraint is active, then
    % calculate the gradient of function g2
    if ~firstConstraintActive
        grad_g = [-1.6*x0(1) -2];
    end
    
    % plot the gradient of the current constraint function g
    quiver(x0(1), x0(2), grad_g(1)/norm(grad_g), grad_g(2)/norm(grad_g))
    hold on
    
    A = [1 grad_f(1) grad_f(2); 1 -grad_g(1) -grad_g(2)];
