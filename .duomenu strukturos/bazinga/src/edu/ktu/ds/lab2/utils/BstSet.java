...
public class BstSet<E extends Comparable<E>> implements SortedSet<E>, Cloneable {
    ...
    public E higher(E e){
        ...
        BstNode<E> node = getHigher(root,e);
        if(node == null)
            return null;
        return node.element;
    }
    
    private BstNode<E> getHigher(BstNode<E> node, E e) {
        if(node == null)
            return null;
        
        if(c.compare(node.element, e) < 0){
            BstNode<E> next = getHigher(node.left, e);
            if(next != null)
                return next;
            else
                return node;
    ...
    public E floor(E e){
        ...
        return floorRecursive(e, root);
    }
    
    private E floorRecursive(E e, BstNode<E> node){
        if(node == null)
            return null;
        if(c.compare(node.element, e) == 0)
            return node.element;
        
        if(c.compare(node.element, e) > 0) // node.element > e
            return floorRecursive(e, node.left);
       
        E value = floorRecursive(e, node.right);
        
        if(value == null)
            return node.element;
        return (c.compare(value, e) <= 0) ? value : node.element;
    }
